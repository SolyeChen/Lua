...
//确保栈上至少有 n 个元素，以便进行后续的操作
#define api_checknelems(L, n)	api_check(L, (n) <= (L->top - L->base))

//检查一个索引（一般是栈索引）是否是有效的
#define api_checkvalidindex(L, i)	api_check(L, (i) != luaO_nilobject)

//将 Lua 状态机栈的栈顶指针向上移动一位
...
//根据返回值的数量来调整栈中的内容，以适应不同情况下的返回值
#define adjustresults(L,nres) \
    { if (nres == LUA_MULTRET && L->top >= L->ci->top) L->ci->top = L->top; }

//检查返回值的数量是否合法
//L：同样是Lua状态机的指针。
//na：这是实际传递给Lua函数的参数数量。
//nr：这是期望接收的返回值数量。
//L->top：这表示栈顶的指针，指向栈的顶部。
//L->ci->top：ci(CallInfo) 是一个指向当前调用帧的指针，表示Lua函数调用的堆栈帧。top 表示这个堆栈帧的栈顶指针。
